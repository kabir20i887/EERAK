...
// ]
// bx.forEach(item=>{
//     db.collection("foods/sYNT0aiTqEnZ0I7IVmd7/group1").add({
//         cqa:[{
//             answer:'yes',
//             quesion:'is we can add ginger  paste and chicken masala ',
//             by:"ERAK on 7th may",
//         }],
//         item
//     })
...
window.onload = () => {
    const searchDb = async (name, id, typ) => {
        document.querySelector('.ct_bx_search input').value = name
        if (typ === 'groc') {
...
const giveHtmlGroc = (added, name, group, price, qt, priceMain, unitIn, unit, com, img, offer, id, stock, sP, sU, forS) => {
    ...
    `
        ...
        document.querySelector(`.list_food_at_ov .ct_cat_by_list`).insertAdjacentHTML('beforeend', html)
        document.querySelector('.for_groc_full_load').style.display = 'none'

        let ltEl = document.querySelector(`.list_food_at_ov .ct_cat_by_list`).lastChild
    ...
    <p>not available</p>
        ...
        if (window.innerWidth < 400) {
            ...
            document.querySelectorAll('.ct_bx_inner1_groc_itm').forEach(cur => {
                cur.style.margin = '0px 0 0 188px'
            ...
            document.querySelectorAll('.bodyessestials .ct_type_fd_cart_itm2').forEach(cur => {
                cur.style.transform = 'translate(-69px,76px)'
            })
        }
...
`
    ...
    document.querySelector(`.list_food_at_ov .ct_cat_by_list`).insertAdjacentHTML('beforeend', html)
    console.log(fdGroup.split(' ').join(''))
    ...
    if (window.innerWidth < 450) {
        document.querySelectorAll('.ct_img_res_main img').forEach(cur => {
            cur.style.transform = 'translate(10px,-20px)'
            cur.style.width = '186px'

        })
        document.querySelectorAll('.ct_type_bx_item_food img').forEach(cur => {
            cur.style.width = '20%'
        ...
        document.querySelectorAll('.ct_inner_fd1_dt_grid').forEach(cur => {
            console.log(cur)
            cur.style.transform = 'translate(-30px)'
...
const functionAddItm2 = () => {
    ...
    if (document.querySelectorAll(`.itm_food_list_ek`).length < 8) {
            ...
            .then(res => {
                ...
                if (addedItm > 0) {
                    if ((addedItm) !== 0 && (addedItm + 1) !== numItm - addedSelected) {
                        if (addedSelected !== 0) {
                            console.log(addedItm, allFd)
                            lastVisible = res.docs[addedItm - 1].data().item
                        }
                        else {
                            lastVisible = res.docs[addedItm - 1].data().item

                            console.log(res.docs, lastVisible)
                        }

                    } else if ((allFd) !== 0 && (addedItm + 1) !== numItm - addedSelected) {
                        console.log(allFd)
                        lastVisible = res.docs[addedItm - 1].data().item
                        console.log(res.docs, lastVisible)
...
console.log(unitIn)
                                if (locFd !== null && locFd.length !== 0) {
                                    ...
                                    if (!addedNames.includes(name.split(' ').join('').toLowerCase())) {
                                        ...
                                        } (false, name, group, price, 1, priceMain, unitIn, unit, com, img, offer, id, stock, null, null)
...
const functionAddItm = async (cat = false, id1 = 1, grp = 0) => {
    ...
    if (cat) {
        let dbCollection = db.collection(`foods/sYNT0aiTqEnZ0I7IVmd7/group${id1}`)
        allReady = 0
        console.log(allReady)
        const resp = await dbCollection
            .orderBy('item')
        ...
        resp.forEach(cur => {
            ...
            console.log(fdGroup, id1)
            giveChtml(false, name, group, price, 1, includes, img, offer, stock, cook, ratings, time, id, false, true, grp)
        })
        groping = 1
        if (addedSelected !== 0) {
            addedItm = -1
    ...
    if (document.querySelector('.for_ek_fd_full_load').style.display === 'none' || document.querySelector('.for_ek_fd_full_load').style.display === '') {
        ...
        if (document.querySelectorAll(`.itm_food_list_ek`).length < numberItmMust) {
            ...
            dbCollection
                ...
                .then(res => {
                    let lastVisible
                    console.log(res.docs.length, addedItm)
                    if (addedItm <= res.docs.length) {
                        if (addedItm > 0) {
                            if ((addedItm) !== 0 && (addedItm + 1) !== numItm - addedSelected) {
...
async function loadCat() {
    ...
    } else if (document.querySelector('.bodyessestials') !== null) {
        let res = await db.collection("grocery").get()
        let addedNames = []
        try {
            res.docs.forEach(itm => {
